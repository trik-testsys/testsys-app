...
- Сущность неизменяема и разделена на `X(id, createdAt, version, data)` и полезную нагрузку `XData`.
  Обновление создаёт новый экземпляр через `X.withData { … }`, который сохраняет `version`.
- `version: EntityVersion` — непрозрачный токен оптимистической блокировки. Его выдаёт хранилище, домен
  передаёт его обратно в `update` и никогда не сравнивает и не изменяет. Устаревший токен приводит к ошибке `update`.
