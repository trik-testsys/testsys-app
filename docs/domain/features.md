...
Представляет Задачу. Задача хранит не более двух версий: последнюю зафиксированную и рабочую, которую
редактирует Разработчик. В зависимости от наличия версий Задача находится в одном из состояний:
...
Загрузка нового файла или изменение ожидаемого Вердикта проверки авторского Решения является изменением Задачи, 
к которой он прикреплен (см. **testsys.entity.task**). Пользователь должен быть 
уведомлен о том, что у Задачи появятся незафиксированные изменения.
...
Открепление от Задачи Ресурса является изменением Задачи (см. **testsys.entity.task**).
...
Если тестирование прошло успешно, текущую версию Задачи можно закоммитить.
...
Закоммитить можно только Задачу, текущая версия которой успешно прошла тестирование.
...
имеющую зафиксированную версию, к Туру, если к нему не был предоставлен доступ 
и Задача поддерживает версию TRIK Studio указанную в Туре.
В Туре будет использована последняя *committed* версия Задачи (условия, полигоны при проверке и так далее).
