...
- У Решения (`Solution`) при создании фиксируются все поля — `file`, `language` и `versionBucket`, поэтому
  `update` Решения не поддерживается. Новая версия авторского Решения — это новое Решение с тем же
  `versionBucket`, на которое переставляется `DeveloperSolution.solution`; файл уже сохранённого Решения
  не меняется, потому что на него ссылаются Посылки вида `developerSolutionTest`.
