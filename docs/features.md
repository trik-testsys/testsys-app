...
1) **Uncommitted** – Задача находится в активной фазе изменений.
2) **Tested** – Задача прошла все необходимые проверки, для того, чтобы считаться "готовой".
3) **Committed** – Изменения Задачи "зафиксированы".

Данные состояния прежде всего предназначены для безопасной доработки задачи, при условии, 
что она уже где-то используется. Предполагается использование последней **committed** версии задачи 
...
к которой он прикреплен, в состояние *uncommitted*. Пользователь должен быть 
уведомлен о том, что Задача перейдет в состояние *uncommitted*.
...
Изначально задача находится в состоянии *uncommitted*. 
...
Прикрепление к Задаче Полигона или авторского Решения переводит ее в состояние *uncommitted*.
...
Открепление от Задачи Ресурса переводит ее в состояние *uncommitted*.
...
Добавление новой поддерживаемой версии TRIK Studio переводит Задачу в состояние **uncommitted**.
...
находящуюся в статусе *committed* к Туру, если к нему не был предоставлен доступ 
