...
`@ConfigProperty(name, dynamic)` ставится на свойство интерфейса конфигурации и уточняет ключ и способ получения
значения:

- `name` — сегмент ключа вместо выведенного из имени свойства; пустое значение (по умолчанию) означает kebab-case
  от имени свойства;
- `dynamic = false` (по умолчанию) — значение фиксировано на всё время работы приложения и разрешается один раз
  при старте;
- `dynamic = true` — значение может меняться от вызова к вызову, пока приложение работает.
